...
var require_log = __commonJS({
  "node_modules/.pnpm/yaml@2.8.2/node_modules/yaml/dist/log.js"(exports) {
    ...
    function warn(logLevel, warning6) {
      if (logLevel === "debug" || logLevel === "warn") {
        if (typeof node_process.emitWarning === "function")
          node_process.emitWarning(warning6);
        else
          console.warn(warning6);
      }
    }
  ...
  "node_modules/.pnpm/yaml@2.8.2/node_modules/yaml/dist/compose/composer.js"(exports) {
    ...
    var Composer = class {
      constructor(options = {}) {
        ...
        this.errors = [];
        this.warnings = [];
        this.onError = (source, code, message, warning6) => {
...
${cb}` : comment;
      ...
      *next(token) {
        ...
        switch (token.type) {
          case "directive":
            this.directives.add(token.source, (offset, message, warning6) => {
              const pos = getErrorPos(token);
              pos[0] += offset;
              this.onError(pos, "BAD_DIRECTIVE", message, warning6);
            });
...
var require_public_api = __commonJS({
  "node_modules/.pnpm/yaml@2.8.2/node_modules/yaml/dist/public-api.js"(exports) {
    ...
    function parse(src, reviver, options) {
      ...
      if (!doc)
        return null;
      doc.warnings.forEach((warning6) => log.warn(doc.options.logLevel, warning6));
...
// src/index.ts
import { saveState as saveState2, getState as getState2, setFailed as setFailed2, info as info6, setOutput as setOutput2, warning as warning5 } from "@actions/core";
import { getExecOutput as getExecOutput3 } from "@actions/exec";
...
// src/types.ts
var RunInstallSchema = external_exports.object({
  cwd: external_exports.string().optional(),
  args: external_exports.array(external_exports.string()).optional()
});
var RunInstallInputSchema = external_exports.union([
  external_exports.null(),
  external_exports.boolean(),
  RunInstallSchema,
  external_exports.array(RunInstallSchema)
]);
var PACKAGE_NAME = "vite-plus-cli";
...
function getInputs() {
  return {
    ...
    cacheDependencyPath: getInput("cache-dependency-path") || void 0
  };
}
function parseRunInstall(input) {
...
async function installVitePlus(inputs) {
...
async function ensureGlobalBinInPath() {
  try {
...
// src/run-install.ts
import { startGroup, endGroup, setFailed, info as info2 } from "@actions/core";
import { exec as exec2 } from "@actions/exec";
async function runViteInstall(inputs) {
  for (const options of inputs.runInstall) {
    ...
    try {
      const exitCode = await exec2("vite", args, {
        cwd,
...
import { warning as warning3, info as info4, debug as debug3, saveState, setOutput } from "@actions/core";
import { arch, platform } from "node:os";

// src/utils.ts
import { info as info3, warning as warning2, debug as debug2 } from "@actions/core";
import { getExecOutput as getExecOutput2 } from "@actions/exec";
import { existsSync, readdirSync } from "node:fs";
import { isAbsolute, join, basename } from "node:path";
var LOCK_FILES = [
  { filename: "pnpm-lock.yaml", type: "pnpm" /* Pnpm */ },
  { filename: "package-lock.json", type: "npm" /* Npm */ },
  { filename: "npm-shrinkwrap.json", type: "npm" /* Npm */ },
  { filename: "yarn.lock", type: "yarn" /* Yarn */ }
];
function detectLockFile(explicitPath) {
  ...
  for (const lockInfo of LOCK_FILES) {
    if (workspaceContents.includes(lockInfo.filename)) {
      ...
      info3(`Auto-detected lock file: ${lockInfo.filename}`);
      return {
        type: lockInfo.type,
        path: fullPath,
...
function inferLockFileType(fullPath, filename) {
  if (filename.includes("pnpm")) {
    return { type: "pnpm" /* Pnpm */, path: fullPath, filename };
  }
  if (filename.includes("yarn")) {
    return { type: "yarn" /* Yarn */, path: fullPath, filename };
  }
  return { type: "npm" /* Npm */, path: fullPath, filename };
}
async function getCacheDirectories(lockType) {
  switch (lockType) {
    case "npm" /* Npm */:
    case "pnpm" /* Pnpm */:
    case "yarn" /* Yarn */:
...
async function getCommandOutput(command, args) {
  ...
  try {
    const result = await getExecOutput2(command, args, {
      silent: true,
      ignoreReturnCode: true
    });
    ...
    debug2(`Command "${cmdStr}" exited with code ${result.exitCode}`);
    return void 0;
  } catch (error) {
    warning2(`Failed to run "${cmdStr}": ${error}`);
    return void 0;
  }
}
async function getViteCacheDir() {
...
async function restoreCache(inputs) {
  ...
  if (!lockFile) {
    warning3("No lock file found. Skipping cache restore.");
    setOutput("cache-hit" /* CacheHit */, false);
    return;
  }
  info4(`Using lock file: ${lockFile.path}`);
  const cachePaths = await getCacheDirectories(lockFile.type);
  if (!cachePaths.length) {
    warning3("No cache directories found. Skipping cache restore.");
    setOutput("cache-hit" /* CacheHit */, false);
    return;
  }
  debug3(`Cache paths: ${cachePaths.join(", ")}`);
  saveState("CACHE_PATHS" /* CachePaths */, JSON.stringify(cachePaths));
  ...
  debug3(`Primary key: ${primaryKey}`);
  debug3(`Restore keys: ${restoreKeys.join(", ")}`);
  saveState("CACHE_PRIMARY_KEY" /* CachePrimaryKey */, primaryKey);
  const matchedKey = await restoreCacheAction(cachePaths, primaryKey, restoreKeys);
  if (matchedKey) {
    info4(`Cache restored from key: ${matchedKey}`);
    saveState("CACHE_MATCHED_KEY" /* CacheMatchedKey */, matchedKey);
    setOutput("cache-hit" /* CacheHit */, true);
  } else {
    info4("Cache not found");
    setOutput("cache-hit" /* CacheHit */, false);
  }
}
...
import { getState, info as info5, warning as warning4 } from "@actions/core";
async function saveCache() {
  const primaryKey = getState("CACHE_PRIMARY_KEY" /* CachePrimaryKey */);
  const matchedKey = getState("CACHE_MATCHED_KEY" /* CacheMatchedKey */);
  const cachePathsJson = getState("CACHE_PATHS" /* CachePaths */);
  if (!primaryKey) {
    info5("No cache key found. Skipping cache save.");
    return;
  }
  if (!cachePathsJson) {
    info5("No cache paths found. Skipping cache save.");
    return;
  }
  if (primaryKey === matchedKey) {
    info5(`Cache hit on primary key "${primaryKey}". Skipping save.`);
    return;
  }
  const cachePaths = JSON.parse(cachePathsJson);
  if (!cachePaths.length) {
    info5("Empty cache paths. Skipping cache save.");
    return;
  }
  try {
    const cacheId = await saveCacheAction(cachePaths, primaryKey);
    if (cacheId === -1) {
      warning4("Cache save failed or was skipped.");
      return;
    }
    info5(`Cache saved with key: ${primaryKey}`);
  } catch (error) {
    warning4(`Failed to save cache: ${error}`);
  }
}

// src/index.ts
async function runMain(inputs) {
  saveState2("IS_POST" /* IsPost */, "true");
...
async function printViteVersion() {
  try {
    const result = await getExecOutput3("vite", ["--version"], { silent: true });
    const versionOutput = result.stdout.trim();
    info6(versionOutput);
    const globalMatch = versionOutput.match(/Global:\s*v?([\d.]+[^\s]*)/i);
    const version = globalMatch?.[1] || "unknown";
    saveState2("INSTALLED_VERSION" /* InstalledVersion */, version);
    setOutput2("version" /* Version */, version);
  } catch (error) {
    warning5(`Could not get vite version: ${error}`);
    setOutput2("version" /* Version */, "unknown");
  }
}
...
async function main() {
  ...
  if (getState2("IS_POST" /* IsPost */) === "true") {
...
main().catch((error) => {
  console.error(error);
  setFailed2(error instanceof Error ? error.message : String(error));
});
