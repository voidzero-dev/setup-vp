...
export function createLockfileResolver(context: ResolutionContext) {
   ...
   */
  function tryResolveVitePlusVersionFromLockfile(
    projectDir: string,
  ...
  function detectLockfileForProject(
    ...
    if (cacheDependencyPath) {
      return detectLockFile(cacheDependencyPath, projectDir);
    }
    const boundary = getWorkspaceDir();
    let dir = projectDir;
    for (;;) {
      const lock = detectLockFile(undefined, dir);
      if (lock) return lock;
   ...
   */
  function parseVitePlusVersionFromLockfile(lock: LockFileInfo, subPath = ""): string | undefined {
  ...
  return { tryResolveVitePlusVersionFromLockfile, parseVitePlusVersionFromLockfile };
}
